...
public class RavenDB_2627Test extends RemoteClientTest {
    ...
    public void shouldKeepPullingDocsAfterServerRestart() throws InterruptedException {
        ...
        try {
            ...
            subscription.addBeforeAcknowledgmentHandler(new EventHandler<VoidArgs>() {
                @Override
                public void handle(Object sender, VoidArgs event) {
    ...
    public void shouldAllowToOpenSubscriptionIfClientDidntSentAliveNotificationOnTime() throws InterruptedException {
        try (IDocumentStore store = new DocumentStore(getDefaultUrl(), getDefaultDb()).initialize()) {
            ...
            connectionOptions.getBatchOptions().setAcknowledgmentTimeout(2000L);
            Subscription<RavenJObject> subscription = store.subscriptions().open(id, connectionOptions);
            store.changes().waitForAllPendingSubscriptions();
            ...
            subscription.addBeforeAcknowledgmentHandler(new EventHandler<VoidArgs>() {
                @Override
                public void handle(Object sender, VoidArgs event) {
                    TimeUtils.cleanSleep(2000);
                }
            }); // to force ACK timeout

            subscription.addAfterAcknowledgmentHandler(new EventHandler<Subscription.LastProcessedEtagEventArgs>() {
                @Override
                public void handle(Object sender, Subscription.LastProcessedEtagEventArgs event) {
                    TimeUtils.cleanSleep(20000);// to prevent the client from sending client-alive notification
                }
            });
            ...
            assertNull(docs.poll(2, TimeUnit.SECONDS)); // make sure that first subscriber didn't get new doc
        }
    }
